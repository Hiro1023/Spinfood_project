...
public class PaerchenListe {
    ...
    private TeilnehmendeList teilnehmendeList;
    ...
    public double calculateSexDiversity() {
        ...
        for (Paerchen pair : paerchenList) {
            if (pair.getTeilnehmer1().getSex() == Geschlecht.male) {
                numMales++;
            } else if (pair.getTeilnehmer1().getSex() == Geschlecht.female) {
                numFemales++;
            } else {
                numOther++;
            }
    
            if (pair.getTeilnehmer2().getSex() == Geschlecht.male) {
                numMales++;
            } else if (pair.getTeilnehmer2().getSex() == Geschlecht.female) {
                numFemales++;
            } else {
                numOther++;
            }
